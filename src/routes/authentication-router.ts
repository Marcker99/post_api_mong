...
import {
    checkLoginOrEmail, checkPassword, checkConfirmCode, checkEmailExistingBeforeReg, checkLoginExistingBeforeReg,
    emailConfirmation
} from "./middleWares/validators/LogAuth_validator";
...
authenticationRouter.post('/login', checkLoginOrEmail, checkPassword, errorsMiddleware,
    async (req: Request, res: Response) => {
        ...
        if (user) {
            ...
            res
                .cookie('refreshToken', refToken, {secure: true, httpOnly: true})
                .status(200)
                .send({accessToken})
        } else {
            res.sendStatus(401)
...
authenticationRouter.post('/refresh-token', checkRefreshToken, async (req: Request, res: Response) => {
    ...
    return res
        .cookie('refreshToken', result.refreshToken, {secure: true, httpOnly: true})
...
authenticationRouter.post('/logout', checkRefreshToken, async (req: Request, res: Response) => {
    ...
    await jwtService.deactivateRefreshToken(refresh_token)
